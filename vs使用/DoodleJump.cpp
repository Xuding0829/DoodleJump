...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	sta = (HBITMAP)LoadImage(NULL, "res/sta.bmp", IMAGE_BITMAP, winX, winY, LR_LOADFROMFILE);
	sta2 = (HBITMAP)LoadImage(NULL, "res/sta2.bmp", IMAGE_BITMAP, winX, winY, LR_LOADFROMFILE);
	for (int i = 0; i < N; i++)
	{
		if (i == 3)
...
void MyPaint(HDC hdc)
{
	...
	{ // 菜单界面
		...
		sprintf_s(sc, "您历史最高得分%d", maxScore);
		TextOut(hdc, 10, 10, sc, lstrlen(sc));

		// 尝试打开文件以读取历史总板块
		err = fopen_s(&fp, filename2, "r");
		if (err != 0 || fp == NULL) {
			// 如果文件不存在，则创建文件
			err = fopen_s(&fp, filename2, "w");
			if (err == 0 && fp != NULL) {
				fclose(fp);
			}
			totol = 0;  // 设置一个默认值
	...
	{ // 游戏结束，显示得分
		...
		errno_t err;
		// 修改最高得分
		err = fopen_s(&fp, filename, "r+"); // 尝试以读写模式打开文件
		if (err != 0 || fp == NULL) {
			// 如果文件打开失败，尝试以写模式创建文件
			err = fopen_s(&fp, filename, "w");
			if (err != 0 || fp == NULL) {
		...
		// 修改总板块
		err = fopen_s(&fp, filename2, "r+"); // 尝试以读写模式打开文件
		if (err != 0 || fp == NULL) {
			// 如果文件打开失败，尝试以写模式创建文件
			err = fopen_s(&fp, filename2, "w");
			if (err != 0 || fp == NULL) {
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		// mciSendString("open res/ms.wav", NULL, 0, NULL);
		// mciSendString("play res/ms.wav repeat", NULL,0, NULL);
		PlaySound(("res/ms.wav"), NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
		sound = 1;
		break;
	case WM_LBUTTONDOWN:
	...
	case WM_DESTROY: // 窗口结束消息
		...
		sound = 0;
		//mciSendString("stop res/ms.wav", 0, 0, 0);
		PostQuitMessage(0);
		break;
	default: // 其他消息
